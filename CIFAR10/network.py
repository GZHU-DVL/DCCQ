...
class DCCQ(nn.Module):
    def __init__(self, feat_dim, M, K, alpha=1, trainable_layer_num=0, queue_size=512, CNN_model_path=None):
        super(DCCQ, self).__init__()
        if CNN_model_path:
        ...
        assert trainable_layer_num <= 6
        for i, param in enumerate(self.vgg.parameters()):
            if (i + trainable_layer_num * 2) < 36:
